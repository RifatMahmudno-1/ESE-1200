...
export default () => {
	if (!existsSync('./src/homeworks'))
		mkdirSync('./src/homeworks', { recursive: true })

	let indexHtml = `# Homeworks
...
`
	for (let homework of homeworks) {
		indexHtml += `- [${homework.title_text}](/homeworks/${
			homework.title_anchor
		}) - Total Projects done: ${homework.items.length}
	${homework.items
		.map(
			item =>
				`- [${item.title}](/homeworks/${homework.title_anchor}#${item.title})`
		)
		.join('\n  ')}
`
	}
	writeFileSync('./src/homeworks/index.md', indexHtml, { encoding: 'utf-8' })
...
<h1>${homework.title_text}</h1>`
		...
		writeFileSync(`./src/homeworks/${homework.title_anchor}.md`, html)
	}
}
